...
public class StopOverrideCommand {

    private final TabExecutor executor;

    public StopOverrideCommand() {
    ...
    public void register(Plugin plugin) {
        CommandBuilder.registerCommand(
                plugin,
                "stop-warning",
                "Warning override for the /stop command",
                "stop-warning",
    ...
    public boolean execute(CommandContext ctx) {
        if (ctx.sender instanceof Entity) {
            TextComponent message = new TextComponent("Use /minecraft:stop to stop the server\nDid you mean /reset?");
            message.setColor(ChatColor.RED);
            ctx.sender.sendMessage(message);
            return true;
        } else {
            return Bukkit.getServer().dispatchCommand(ctx.sender, "minecraft:stop");
        }
    }
}
